...
# 📋 Easy Risk Tracker Scoring Methodology
...
|------|-------------|----------------|
| 🟢 **Low Risk** | 7.5 - 10.0 | Reasonable for consideration with standard due diligence |
| 🟡 **Moderate Risk** | 6.0 - 7.4 | Approach with caution, monitor closely, limit exposure |
| 🟠 **High Risk** | 4.0 - 5.9 | Significant concerns, consider avoiding or minimal exposure only |
| 🔴 **Critical Risk** | 0.0 - 3.9 | Severe red flags, strong recommendation to avoid |

### Automatic Critical Risk Triggers

Regardless of numerical score, the following automatically escalate to **🔴 Critical Risk**:
...
```
Final Score = (Leadership × 0.15) + (Tokenomics × 0.15) + (Earning × 0.10) + 
              (Liquidity × 0.20) + (Community × 0.10) + (Technical × 0.15) + 
              (Transparency × 0.10) + (Personal × 0.05)
```
