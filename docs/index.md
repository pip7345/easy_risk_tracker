---
sidebar_position: 1
title: Project Risk Scores Overview
slug: /
---

# 📊 Easy Risk Tracker - Project Assessments

Welcome to the Easy Risk Tracker comprehensive project risk assessment database. This resource provides detailed analysis of cryptocurrency and Web3 projects using our standardized 8-category scoring methodology.

## Risk Tier Classification

| Tier | Score Range | Description |
|------|-------------|-------------|
| 🟢 **Low Risk** | 7.5 - 10.0 | Strong fundamentals, transparent operations, minimal red flags |
| 🟡 **Moderate Risk** | 6.0 - 7.4 | Generally sound with some concerns requiring monitoring |
| 🟠 **High Risk** | 4.0 - 5.9 | Significant concerns present, proceed with extreme caution |
| 🔴 **Critical Risk** | 0.0 - 3.9 | Severe red flags, potential fraud indicators, avoid |

---

## 📋 All Project Scores

| Project | Category | Score | Risk Tier | Key Concerns |
|---------|----------|-------|-----------|--------------|
| [Bellator](projects/bellator) | Investment/MLM | 2.8 | 🔴 Critical | Anonymous leadership, unrealistic returns, legal risks |
| [XUSD X1](projects/xusd_x1) | Stablecoin/DeFi | 0.5 | 🔴 Critical | Confirmed federal fraud case, site active |
| [Gradient](projects/gradient) | DePIN/Computing | 6.4 | 🟡 Moderate | Needs more liquidity and adoption data |
| [Grass](projects/grass) | DePIN/Network | 7.2 | 🟡 Moderate | Strong concept, liquidity metrics unclear |
| [Uprock](projects/uprock) | DePIN/Bandwidth | 6.5 | 🟡 Moderate | Early-stage adoption metrics limited |
| [Silencio](projects/silencio) | DePIN/Data | 6.8 | 🟡 Moderate | Sound concept, needs broader validation |
| [Tangem](projects/tangem) | Hardware Wallet | 7.8 | 🟢 Low | Established product, verifiable team |
| [DeNet](projects/denet) | DePIN/Storage | 6.2 | 🟡 Moderate | Some centralization concerns |
| [Kast Card](projects/kast_card) | Crypto Card | 6.09 | 🟡 Moderate | Regulatory clarity needed |
| [Builder](projects/builder) | No-Code/Web3 | 2.16 | 🔴 Critical | Extreme lockups, no verified team |
| [Atleta Network](projects/atleta_network) | Sports/Blockchain | 6.28 | 🟡 Moderate | Leadership visibility and liquidity unclear |
| [Blockchain Sports](projects/blockchain_sports_ecosystem) | Sports/Web3 | 6.18 | 🟡 Moderate | Execution risk across many products |
| [Amplivio Corsair](projects/amplivio_corsair) | Fintech/CSR Token | 4.50 | 🟠 High | Referral-heavy model, unverified operations |
| [ZPLN](projects/zpln) | DeFi/Platform | 3.94 | 🔴 Critical | Team transparency and liquidity concerns |
| [Texit](projects/texit) | Mining/L1 | 3.19 | 🔴 Critical | Centralized mining claims, thin liquidity |
| [WeFi](projects/wefi) | DeFi/Lending | 3.76 | 🔴 Critical | No team disclosure, no updates |
| [Cha-Ching](projects/cha_ching) | AI/White-Label | 3.5 | 🔴 Critical | Stalled development, Bellator dependency |
| [Miracle Cash](projects/miracle_cash) | DeFi/Payment | 2.5 | 🔴 Critical | Urgency tactics, opaque partnerships |
| [iiZR Playbook](projects/iizr_playbook) | Education/Tools | 4.48 | 🟠 High | Limited transparency, unclear value |
| [Veritas Vault](projects/veritas_vault) | DeFi/DAO | 2.0 | 🔴 Critical | Withdrawals paused, extreme APY |
| [BG Wealth Sharing](projects/bg_wealth_sharing) | Investment/MLM | 3.0 | 🔴 Critical | False institutional claims, MLM structure |
| [Bitton](projects/bitton) | L1/Consensus | 2.77 | 🔴 Critical | Long lockups, unclear reward source |
| [SQWL](projects/sqwl) | Education Platform | 4.48 | 🟠 High | Limited verification of claims |
| [Masterwealthbuilders](projects/masterwealthbuilders) | Financial Services/MLM | 4.0 | 🟠 High | Impossible investment claims |
| [XSynergy](projects/xsynergy) | Investment/Matrix | 1.64 | 🔴 Critical | Matrix compensation, no verified team |
| [Zionix Global](projects/zionix_global) | Investment/Forex | 1.15 | 🔴 Critical | Unverified AI trading claims |
| [Tereth](projects/tereth) | DeFi/Token | 4.08 | 🟠 High | High tax tokenomics, unclear audits |
| [Sosana](projects/sosana) | Meme/Token | 5.05 | 🟠 High | Meme speculation, leadership unclear |
| [QuantumFi Global](projects/quantumfi_global) | Fintech/Payments | 4.79 | 🟠 High | No team bios or licensing proof |
| [My Business Club](projects/my_business_club) | Education/MLM | 1.73 | 🔴 Critical | Heavy MLM structure, low transparency |
| [Mario Lundqvist](projects/mario_lundqvist) | Trading Education | 2.55 | 🔴 Critical | Limited verification, AI trading claims |
| [D.AI.SY](projects/daisey) | AI Trading | 3.85 | 🔴 Critical | Opaque performance, unclear leadership |
| [BehindMLM](projects/behindmlm) | Review Site | N/A | 📰 Review | Informational watchdog site |

---

## 🎯 Scoring Methodology Summary

Each project is evaluated across **8 weighted categories**:

| Category | Weight | Focus Areas |
|----------|--------|-------------|
| Leadership & Team | 15% | Identity verification, experience, track record |
| Tokenomics | 15% | Distribution, utility, supply mechanisms |
| Earning Mechanism | 10% | Revenue source legitimacy, sustainability |
| Liquidity & Market | 20% | Trading volume, exchange listings, price stability |
| Community & Reputation | 10% | Engagement authenticity, reviews, warnings |
| Technical Quality | 15% | Code audits, security, innovation |
| Transparency | 10% | Documentation, communication, financial disclosure |
| Personal Assessment | 5% | Investment thesis, risk tolerance alignment |

[📋 View Full Methodology](methodology)

---

## ⚠️ Important Disclaimers

:::warning Not Financial Advice
This risk assessment tool is for **educational purposes only**. Always conduct your own research (DYOR) before making any investment decisions. The scores provided represent our analysis at the time of assessment and may change as new information becomes available.
:::

:::info Dynamic Assessments
Project scores are subject to revision based on:
- New information or developments
- Community feedback and reports
- Changes in regulatory environment
- Updates to project fundamentals
:::

---

## 🔄 Last Updated

**Assessment Date:** January 25, 2026  
**Framework Version:** 1.1  
**Projects Assessed:** 33

---

## 📬 Report Issues

If you have information that may affect a project's assessment, please submit documentation for review. Anonymous tips are accepted, but verifiable sources receive priority consideration.
